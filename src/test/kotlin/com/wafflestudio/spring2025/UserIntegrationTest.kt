    ...
    constructor(
        ...
        fun `이름을 변경하면 200과 변경된 이름을 반환한다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = newName, email = null, password = null, profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isOk)
        ...
        fun `비밀번호를 변경하면 이전 비밀번호는 무효화되고 새 비밀번호로 로그인할 수 있다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = null, email = null, password = newPassword, profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isOk)
        ...
        fun `S3에 존재하는 키로 프로필 이미지를 변경하면 200을 반환한다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = null, email = null, password = null, profileImage = validKey)))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isOk)
                .andExpect(jsonPath("$.profileImage").value(presignedUrl))
        }
        ...
        fun `이름과 비밀번호를 동시에 변경하면 200과 변경된 정보를 반환한다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = newName, email = null, password = newPassword, profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isOk)
                .andExpect(jsonPath("$.name").value(newName))
                .andExpect(jsonPath("$.email").value(user.email))
        ...
        fun `이메일 변경 시도 시 403을 반환한다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = null, email = "newemail@example.com", password = null, profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
        ...
        fun `이름을 공백 문자열로 변경 시도 시 400을 반환한다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = "   ", email = null, password = null, profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
        ...
        fun `비밀번호를 8자 미만으로 변경 시도 시 400을 반환한다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = null, email = null, password = "abc1", profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isBadRequest)
        ...
        fun `비밀번호에 숫자가 없으면 변경 시도 시 400을 반환한다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = null, email = null, password = "onlyletters", profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isBadRequest)
                .andExpect(jsonPath("$.code").value("BAD_PASSWORD"))
        }
        ...
        fun `비밀번호에 문자가 없으면 변경 시도 시 400을 반환한다`() {
            ...
            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = null, email = null, password = "12345678", profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isBadRequest)
                .andExpect(jsonPath("$.code").value("BAD_PASSWORD"))
        }
        ...
        fun `S3에 존재하지 않는 키로 프로필 이미지 변경 시도 시 400을 반환한다`() {
            ...
            given(s3Client.headObject(any(HeadObjectRequest::class.java)))
                .willThrow(NoSuchKeyException.builder().message("Not found").statusCode(404).build())

            mvc.perform(
                patch("/api/users/me")
                    .header("Authorization", "Bearer $token")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = null, email = null, password = null, profileImage = "nonexistent/image.jpg")))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isBadRequest)
        ...
        fun `토큰 없이 내 정보 수정 시도 시 401을 반환한다`() {
            mvc.perform(
                patch("/api/users/me")
                    .content(mapper.writeValueAsString(PatchMeRequest(name = "새이름", email = null, password = null, profileImage = null)))
                    .contentType(MediaType.APPLICATION_JSON),
            ).andExpect(status().isUnauthorized)
                .andExpect(jsonPath("$.code").value("AUTHENTICATION_REQUIRED"))
        }
